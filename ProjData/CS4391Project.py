...
try: 
    mkdir('ProjData/resize200') 
    mkdir('ProjData/resize50')
...
for gray in grays:
    ...
    cv.imwrite('ProjData/resize200/' + str(count) + '.jpg', g200)
    cv.imwrite('ProjData/resize50/' + str(count) + '.jpg', g50)
    count += 1
...
for i in range(len(train_sift)):
    np.savetxt('ProjData/SIFT/siftData' + str(i) +'.csv', train_sift[i], delimiter=',')
...
try: 
    mkdir('ProjData/histograms')
except OSError as error: pass

for i in range(len(hist_feat)):
    np.savetxt('ProjData/histograms/histData' + str(i) +'.csv', hist_feat[i], delimiter=',')
